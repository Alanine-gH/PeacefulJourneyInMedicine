  ...
  <div class="page-wrap">
    ...
    <div class="table-card">
      <table class="data-table">
        <thead>
          <tr>
            <th>ID</th><th>用户ID</th><th>姓名</th><th>手机号</th>
            <th>性别</th><th>与账户关系</th><th>是否默认</th>
            <th>状态</th><th>创建时间</th><th>操作</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="loading"><td colspan="10" class="empty">加载中...</td></tr>
          <tr v-else-if="list.length===0"><td colspan="10" class="empty">暂无数据</td></tr>
          <tr v-for="row in list" :key="row.id">
            ...
            <td>{{ row.relation || '-' }}</td>
            <td><span class="tag" :class="row.isDefault===1?'tag-green':'tag-gray'">{{ row.isDefault===1?'默认':'否' }}</span></td>
            <td><span class="tag" :class="row.status===1?'tag-green':'tag-red'">{{ row.status===1?'正常':'停用' }}</span></td>
    ...
    <div v-if="dialog.visible" class="modal-mask" @click.self="dialog.visible=false">
      <div class="modal-box">
        ...
        <div class="modal-body">
          ...
          <div class="form-row"><label>与账户关系</label><input v-model="form.relation" placeholder="如：本人/父母/子女"></div>
          <div class="form-row"><label>地址</label><input v-model="form.address" placeholder="地址"></div>
          <div class="form-row"><label>是否默认</label><span class="form-readonly">{{ form.isDefault===1?'是（默认）':'否' }}<span class="form-readonly-tip">（请通过列表「设默认」按钮修改）</span></span></div>
          <div class="form-row"><label>状态</label>
            <select v-model="form.status"><option :value="1">正常</option><option :value="2">停用</option></select>
          </div>
    ...
    <div v-if="detail.visible" class="modal-mask" @click.self="detail.visible=false">
      <div class="modal-box">
        <div class="modal-header"><span>就诊人详情</span><button @click="detail.visible=false">✕</button></div>
...
export default {
  data() {
    return {
      ...
      form: { userId:'', name:'', phone:'', idCard:'', gender:0, relation:'', address:'', isDefault:2, status:1, remark:'' },
  ...
  methods: {
    ...
    openAdd() {
      this.form = { userId:'', name:'', phone:'', idCard:'', gender:0, relation:'', address:'', isDefault:2, status:1, remark:'' }
    ...
    async submitForm() {
      if (!this.form.name) { alert('姓名不能为空'); return }
      if (!this.form.userId) { alert('用户ID不能为空'); return }
      let res
...
.btn-edit { background: #fefce8; color: #b45309; }
.btn-del { background: #fef2f2; color: #dc2626; }
.btn-default { background: #f0fdf4; color: #16a34a; }
.form-readonly { font-size: 13px; color: var(--text-primary); display: flex; align-items: center; gap: 8px; }
.form-readonly-tip { font-size: 12px; color: #aaa; }
...
.form-row label { width: 100px; font-size: 13px; color: var(--text-muted); flex-shrink: 0; }
.form-row input, .form-row select { flex: 1; padding: 8px 12px; border: 1px solid var(--border-input); border-radius: 6px; font-size: 13px; background: var(--bg-input); color: var(--text-primary); }
...
.detail-body { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
