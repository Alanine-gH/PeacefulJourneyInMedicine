...
public class OrderEvaluationServiceImpl extends ServiceImpl<OrderEvaluationMapper, OrderEvaluation>
    ...
    @Override
    public List<String> getEvaluatedOrderNos(List<String> orderNos) {
        if (orderNos == null || orderNos.isEmpty()) {
            return Collections.emptyList();
        }
        return list(new LambdaQueryWrapper<OrderEvaluation>()
                .in(OrderEvaluation::getOrderNo, orderNos)
                .select(OrderEvaluation::getOrderNo))
                .stream()
                .map(OrderEvaluation::getOrderNo)
                .distinct()
    ...
    public void submitEvaluation(com.PJDM.dto.EvaluationSubmitDTO dto) {
        ...
        OrderOrder order = orderOrderMapper.selectOne(
                new LambdaQueryWrapper<OrderOrder>()
                        .eq(OrderOrder::getOrderNo, dto.getOrderNo()));
